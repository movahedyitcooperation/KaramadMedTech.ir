...
| Images | next/image; **admin-uploaded** product images live on the backend (`backend/uploads/`, served at `/api/v1/uploads/*` with a long-lived `Cache-Control`), resolved via `BACKEND_PUBLIC_ORIGIN` — see §6; storefront/brand assets stay in `/public`. Backend-side, every upload is re-encoded to a single size-capped WebP (long edge ≤1600px) via **Pillow** (`backend/app/api/v1/admin_uploads.py`) — the one exception to "no dependencies not listed here," added after asking (see `docs/BACKEND-GAPS.md`) |
